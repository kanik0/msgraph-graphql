...
function parseMetadata(metadata){
    ...
    let enums = parseEnums([].concat.apply([], schema.map(function (el) { return el['EnumType']; }))); //mapping from name to definition (node -> def)
    let complexResponse =  parseTypes([].concat.apply([], schema.map(function (el) { return el['ComplexType']; })), false); //mapping from name to schema (node -> set of properties)
    let entityResponse = parseTypes([].concat.apply([], schema.map(function (el) { return el['EntityType']; })), true);
    ...
    let entityContainer = schema[1]['EntityContainer'];
